...
function obtenerSemanaLunes($fecha, $respaldo = "Sin Datos")
{
    if (!$fecha || $fecha === "0000-00-00") {
        return $respaldo;
    }
    $fechaSemana = DateTime::createFromFormat('Y-m-d', $fecha);
    if (!$fechaSemana) {
        $fechaSemana = DateTime::createFromFormat('Y-m-d H:i:s', $fecha);
    }
    if (!$fechaSemana) {
        return $respaldo;
    }
    return $fechaSemana->format('W');
}
                                            ...
                                            <table id="detalladoicarga" class=" table-hover " style="width: 100%;">
                                                ...
                                                <tbody>
                                                    <?php foreach ($ARRAYICARGA as $r) : ?>
                                                        ...
                                                        $SEMANAICARGA = obtenerSemanaLunes($r['FECHA'], $r['SEMANA']);
                                                        $SEMANACORTEDOCUMENTAL = obtenerSemanaLunes($r['FECHACORTEDOCUMENTAL'], $r['SEMANACORTEDOCUMENTAL']);
                                                        $SEMANAETD = obtenerSemanaLunes($r['FECHAETD'], $r['SEMANAETD']);
                                                        $SEMANAETA = obtenerSemanaLunes($r['FECHAETA'], $r['SEMANAETA']);
                                                        $SEMANAETAREAL = obtenerSemanaLunes($r['FECHAETAREAL'], $r['SEMANAETAREAL']);
                                                        ?>
                                                       ...
                                                       <?php foreach ($ARRAYDCARGA as $s) : ?>
                                                            ...
                                                            <tr class="text-center">
                                                                ...
                                                                <td> <?php echo $SEMANAICARGA; ?> </td>
                                                                <td> <?php echo $SEMANACORTEDOCUMENTAL; ?> </td>
                                                                <td> <?php echo $SEMANAETD; ?> </td>
                                                                <td> <?php echo $SEMANAETA; ?> </td>
                                                                <td> <?php echo $SEMANAETAREAL; ?> </td>
