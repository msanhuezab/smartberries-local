...
function obtenerDetalleInvoiceEditableV2($idIcarga) {
  try {
    ...
    $stmt = $db->prepare("
      SELECT
        ...
        idet.ID_TCALIBRE,
        IFNULL(cal.NOMBRE_TCALIBRE, 'Sin calibre') AS TCALIBRE,
      ...
      LEFT JOIN fruta_tcalibre cal ON cal.ID_TCALIBRE = idet.ID_TCALIBRE
      WHERE inv.ID_ICARGA = ?
      AND inv.ESTADO_REGISTRO = 1
      AND inv.ESTADO_INVOICE IN ('BORRADOR', 'CONFIRMADA')
      GROUP BY est.ID_ECOMERCIAL, idet.ID_TCALIBRE, idet.PRECIO_CAJA, inv.MONEDA_INVOICE
      ORDER BY NOMBRE, cal.ORDEN, TCALIBRE
    ");
...
function obtenerObservacionInvoiceEditableV2($idIcarga) {
  try {
    $db = BDCONFIG::conectar();
    $stmt = $db->prepare("
...
if($ARRAYICARGA){
    ...
    $ARRAYDCARGA_EDITABLE = obtenerDetalleInvoiceEditableV2($IDOP);
  ...
  endforeach;
    ...
    foreach($ARRAYCLAVESDETALLE as $keyDetalle) {
      ...
      $OBSERVACION_INVOICE_EDITABLE = obtenerObservacionInvoiceEditableV2($IDOP);
