...
function obtenerDetalleInvoiceEditable($idIcarga)
{
  try {
    ...
    $stmt = $db->prepare("
      SELECT
        est.ID_ESTANDAR,
        est.ID_ECOMERCIAL,
        IFNULL(com.CODIGO_ECOMERCIAL, '') AS CODIGO,
        IFNULL(com.NOMBRE_ECOMERCIAL, IFNULL(est.NOMBRE_ESTANDAR, '')) AS NOMBRE,
        SUM(IFNULL(idet.CANTIDAD_ENVASE, 0)) AS ENVASESF,
        SUM(IFNULL(idet.KILOS_NETO, 0)) AS NETOSF,
        SUM(IFNULL(idet.KILOS_BRUTO, 0)) AS BRUTOSF,
        SUM(IFNULL(idet.KILOS_BRUTO, 0)) AS BRUTOSRF,
        FORMAT(SUM(IFNULL(idet.CANTIDAD_ENVASE, 0)), 0, 'de_DE') AS ENVASE,
        FORMAT(SUM(IFNULL(idet.KILOS_NETO, 0)), 2, 'de_DE') AS NETO,
        FORMAT(SUM(IFNULL(idet.KILOS_BRUTO, 0)), 2, 'de_DE') AS BRUTO,
        inv.MONEDA_INVOICE AS TMONEDA,
        IFNULL(dic.ID_TMANEJO, 0) AS ID_TMANEJO,
        IFNULL(tm.NOMBRE_TMANEJO, '') AS TMANEJO,
        IFNULL(idet.PRECIO_CAJA, 0) AS USSF,
        FORMAT(IFNULL(idet.PRECIO_CAJA, 0), 2, 'de_DE') AS US,
        SUM(IFNULL(idet.TOTAL_LINEA, IFNULL(idet.CANTIDAD_ENVASE, 0) * IFNULL(idet.PRECIO_CAJA, 0))) AS TOTALUSSF,
        FORMAT(SUM(IFNULL(idet.TOTAL_LINEA, IFNULL(idet.CANTIDAD_ENVASE, 0) * IFNULL(idet.PRECIO_CAJA, 0))), 2, 'de_DE') AS TOTALUS
      ...
      LEFT JOIN fruta_dicarga dic ON dic.ID_DICARGA = idet.ID_DICARGA
      LEFT JOIN fruta_tmanejo tm ON tm.ID_TMANEJO = dic.ID_TMANEJO
      LEFT JOIN estandar_eexportacion est ON est.ID_ESTANDAR = idet.ID_ESTANDAR
      LEFT JOIN estandar_ecomercial com ON com.ID_ECOMERCIAL = est.ID_ECOMERCIAL
      WHERE inv.ID_ICARGA = ?
      AND inv.ESTADO_REGISTRO = 1
      AND inv.ESTADO_INVOICE IN ('BORRADOR', 'CONFIRMADA')
      GROUP BY est.ID_ECOMERCIAL, idet.PRECIO_CAJA, dic.ID_TMANEJO, inv.MONEDA_INVOICE
      ORDER BY NOMBRE, TMANEJO, idet.PRECIO_CAJA
    ");
    $stmt->execute([(int)$idIcarga]);
...
function obtenerObservacionInvoiceEditable($idIcarga)
{
  try {
    $db = BDCONFIG::conectar();
    $stmt = $db->prepare("
      SELECT OBSERVACION_INVOICE
...
function obtenerCargosInvoice($idIcarga)
{
  try {
    $db = BDCONFIG::conectar();
    $stmt = $db->prepare("
      SELECT SEGURO_INVOICE, FLETE_INVOICE
      FROM exportadora_invoice
      WHERE ID_ICARGA = ?
      AND ESTADO_REGISTRO = 1
      ORDER BY ID_INVOICE DESC
    ...
    ");
    $stmt->execute([(int)$idIcarga]);
    return $stmt->fetch(PDO::FETCH_ASSOC) ?: ['SEGURO_INVOICE' => null, 'FLETE_INVOICE' => null];
  } catch (Exception $e) {
    return ['SEGURO_INVOICE' => null, 'FLETE_INVOICE' => null];
  }
}
...
if ($ARRAYICARGA) {
  ...
  $ARRAYDCARGA_EDITABLE = obtenerDetalleInvoiceEditable($IDOP);
  if ($ARRAYDCARGA_EDITABLE) {
    $ARRAYDCARGA = $ARRAYDCARGA_EDITABLE;
  }
  ...
  $OBSERVACION_INVOICE_EDITABLE = obtenerObservacionInvoiceEditable($IDOP);
  if ($OBSERVACION_INVOICE_EDITABLE !== "") {
    $OBSERVACIONESI = $OBSERVACION_INVOICE_EDITABLE;
  }
  $CARGOS_INVOICE = obtenerCargosInvoice($IDOP);
  $SEGURO_INVOICE = ($CARGOS_INVOICE['SEGURO_INVOICE'] !== null && $CARGOS_INVOICE['SEGURO_INVOICE'] > 0)
    ? (float)$CARGOS_INVOICE['SEGURO_INVOICE'] : 0;
  $FLETE_INVOICE  = ($CARGOS_INVOICE['FLETE_INVOICE']  !== null && $CARGOS_INVOICE['FLETE_INVOICE']  > 0)
    ? (float)$CARGOS_INVOICE['FLETE_INVOICE']  : 0;
...
$_incoterm = strtoupper(trim((string)$NOMBRECVENTA));
$_esCIF = strpos($_incoterm, 'CIF') !== false;
$_esDPU = strpos($_incoterm, 'DPU') !== false;

// Seguro solo para CIF
if ($_esCIF && $SEGURO_INVOICE > 0) {
  $TOTALUSV += $SEGURO_INVOICE;
  $html = $html . '
              <tr>
                <td class="center">-</td>
                <td class="center">Insurance (' . htmlspecialchars($NOMBRECVENTA) . ')</td>
                <td class="center">-</td>
                <td class="center">-</td>
                <td class="center">-</td>
                <td class="center"></td>
                <td class="center">-</td>
                <td class="center">' . number_format($SEGURO_INVOICE, 2, ",", ".") . '</td>
              </tr>
  ';
}

// Flete para CIF y DPU
if (($_esCIF || $_esDPU) && $FLETE_INVOICE > 0) {
  $TOTALUSV += $FLETE_INVOICE;
  $html = $html . '
              <tr>
                <td class="center">-</td>
                <td class="center">Freight (' . htmlspecialchars($NOMBRECVENTA) . ')</td>
                <td class="center">-</td>
                <td class="center">-</td>
                <td class="center">-</td>
                <td class="center"></td>
                <td class="center">-</td>
                <td class="center">' . number_format($FLETE_INVOICE, 2, ",", ".") . '</td>
              </tr>
  ';
}
...
<div style="position:fixed;bottom:10mm;right:12mm;display:table;">
  <div style="border-top:1px solid #555;padding-top:8px;padding-bottom:4px;font-size:9px;font-family:Arial,sans-serif;color:#333;white-space:nowrap;">
    MOREEN WELDT C. <br> Export Manager <br> Volcán Foods Ltda. <br> Fundo El Álamo, Los Ángeles <br>
  </div>
</div>
