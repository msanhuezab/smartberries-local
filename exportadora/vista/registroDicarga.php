    ...
    <!- LLAMADA DE LOS ARCHIVOS NECESARIOS PARA DISEÑO Y FUNCIONES BASE DE LA VISTA -!>
        ...
        <!- FUNCIONES BASES -!>
            <script type="text/javascript">
                ...
                function aplicarSelectPresupuestoDetalle(id, valor) {
                ...
                function cargarFobPresupuesto() {
                    var icarga = document.getElementById('IDP') ? document.getElementById('IDP').value : '';
                    var estandar = document.getElementById('EEXPORTACION') ? document.getElementById('EEXPORTACION').value : '';
                    var calibre = document.getElementById('CALIBRE') ? document.getElementById('CALIBRE').value : '';
                    var precioActual = parseFloat((document.getElementById('PRECIOUS') || {}).value || 0) || 0;
                    if (!icarga || !estandar || !calibre || precioActual > 0) {
                        return;
                    }

                    fetch('ajaxPresupuestoFob.php?accion=precio&icarga=' + encodeURIComponent(icarga) + '&estandar=' + encodeURIComponent(estandar) + '&calibre=' + encodeURIComponent(calibre))
                        .then(function (response) { return response.json(); })
                        .then(function (data) {
                            if (!data.ok) {
                                return;
                            }
                            document.getElementById('PRECIOUS').value = data.fob_caja;
                            aplicarSelectPresupuestoDetalle('TMONEDA', data.id_tmoneda);
                            precio();
                        })
                        .catch(function () {});
                }
                ...
                document.addEventListener('DOMContentLoaded', function () {
                    ['CALIBRE', 'TMONEDA'].forEach(function (id) {
                        var el = document.getElementById(id);
                        if (el) {
                            el.addEventListener('change', function () {
                                if (id === 'CALIBRE') {
                                    cargarFobPresupuesto();
                                }
                            });
                        }
                    });
                                    ...
                                    <?php } ?>
                                        <div class="col-xxl-4 col-xl-4 col-lg-4 col-md-12 col-sm-12 col-12 col-xs-12">
                                            <div class="form-group">
                                                ...
                                                <select class="form-control select2" id="CALIBRE" name="CALIBRE" style="width: 100%;" onchange="cargarFobPresupuesto()" <?php echo $DISABLED; ?>>
                                                    <option></option>
